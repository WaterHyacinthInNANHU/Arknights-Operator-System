...
from connector.util.socketutil import recvall, recvexactly
from . import revconn
...
def find_adb_from_android_sdk():
    ...
    try:
        ...
        adbpath = os.path.join(root, 'platform-dev', base)
...
def ensure_adb_alive():
    ...
    import subprocess
    adb_bin = config.get('device/adb_binary', None)
    if adb_bin is None:
        adb_binaries = ['adb', os.path.join(config.ADB_ROOT, 'adb')]
        find_adb = find_adb_from_android_sdk()
        if find_adb is not None:
            adb_binaries.append(find_adb)
    else:
        adb_binaries = [adb_bin]
    for adb_bin in adb_binaries:
        try:
            logger.debug('trying %r', adb_bin)
...
class ADBConnector:
    ...
    @staticmethod
    def get_sub_screen(image, screen_range):
        return image.crop(
            (
                screen_range[0][0],
                screen_range[0][1],
    ...
    def load_device_config(self):
        ...
        if screenshot_impl == 'safe':
            ...
            self.screencap_impl = _ScreenCapImplReverseLoopback(self.device_session_factory, self.screenshot_rotate,
                                                                self.rch, self.loopback)
    ...
    def _probe_device_config(self, device_record):
        ...
        # try out the available screenshot method
        try:
            ...
            logger.debug('gzipped raw screencap: %dx%d resource in %.02f secs ', screenshot.width, screenshot.height,
                         time_gzipped_raw)
        except:
            ...
            logger.debug('PNG screencap: %dx%d resource in %.03f secs', screenshot.width, screenshot.height,
                         time_gzipped_raw)
        ...
        if workaround_slow_emulator_adb == 'auto' or workaround_slow_emulator_adb == 'always':
            ...
            if len(loopbacks):
                ...
                if self._test_reverse_connection(loopbacks):
                    loopback_impl = _ScreenCapImplReverseLoopback(self.device_session_factory,
                                                                  device_record['screenshot_rotate'], self.rch,
                                                                  self.loopback)
                    t0 = time.monotonic()
                    screenshot = loopback_impl.screencap()
                    t1 = time.monotonic()
                    time_reverse_loopback = t1 - t0
                    logger.debug('reverse connection raw screencap: %dx%d resource in %.03f secs', screenshot.width,
                                 screenshot.height, time_gzipped_raw)
