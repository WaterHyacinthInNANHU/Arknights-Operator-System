...
import traceback
import os
from network.MyWebServerClient import MyWebServerClient


class ArknightsMyserver(object):
    def __init__(self):
        self.ws = MyWebServerClient('ws://101.132.147.44:2000', None, None, 'python', 5, self.onmessage)
        self.ark = ArknightsOperator()
        # 配置文件写入
    ...
    def orderByInner(self,ws,msg,workstate):
        ...
        try:
            ...
            if(msg.find(orderlist[0])!=-1):
                self.send(ws,'message',u'Arkights arknights:收到,打印材料清单')
            ...
            if(msg.find(orderlist[2])!=-1):
                ...
                self.send(ws,'message',u'Arkights arknights:收到,所有命令文件如下')
                for item in filelist:
                    self.send(ws,'message',os.path.splitext(item)[0])
            if(msg.find(orderlist[3])!=-1):
                self.send(ws,'message',u'Arkights arknights:收到,所有内置命令如下')
                for item in orderlist:
                    self.send(ws,'message',str(item))
            if(msg.find(orderlist[4])!=-1):
                self.send(ws,'message',u'Arkights arknights:在')
            # if(msg.find(orderlist[5])!=-1):
            #     loc = r"imgs\\screenshot\\screenshot.jpg"
            #     self.send(ws,'message',u'Arkights arknights:收到，截屏上传中')
    ...
    def orderByFile(self,ws,msg,workstate):
        try:
            if(msg.find('*') == -1):
                msg = msg+'*1'
            self.send(ws,'message',u'Arkights arknights:收到')
            pattern = re.compile(r'(.*)\*')  # 用于匹配order
            order = pattern.match(msg).groups()[0]
            pattern = re.compile(r'.*\*(.*)')  # 匹配次数
            times = pattern.match(msg).groups()[0]
            self.send(ws,'message',u'Arkights arknights:将开始执行'+str(times)+"次文件"+str(order))
        except:
            workstate += ['执行过程中发生错误']
            self.send(ws,'message',u'Arkights arknights:语法错误')
            return
        try:
            ...
            self.send(ws,'message',u'Arkights arknights:全部执行完毕')
        except FileNotFoundError or NotADirectoryError:
            workstate += ['执行过程中发生错误']
            self.send(ws,'message',u'Arkights arknights:没有对应命令')
        except:
            self.send(ws,'message',traceback.format_exc())
            workstate += ['执行过程中发生错误']
            self.send(ws,'message',u'Arkights Operator:执行过程中发生错误')
    ...
    def onmessage(self,ws,data,workstate):
        ...
        try:
            ...
            self.send(ws,'message',u'Arkights arknights:请输入正确的命令格式')
        ...
        try:
            if(type == 'r'):
                ...
                self.send(ws,'message',u'Arkights arknights:请设置正确的命令类型')
...
if __name__ == '__main__':
    Arknights_Myserver = ArknightsMyserver()
