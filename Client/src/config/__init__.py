...
ADB_ROOT = os.path.join(root, '..', 'bin', 'ADB', sys.platform)
SCREEN_SHOOT_SAVE_PATH = os.path.join(root, '..', 'data', 'screenshot')
...
if CONFIG_MODE == 'dev':
    config_file = os.path.join(CONFIG_PATH, 'config-dev.yaml')
elif CONFIG_MODE == 'release':
    config_file = os.path.join(CONFIG_PATH, 'config.yaml')
else:
    config_file = os.path.join(CONFIG_PATH, 'config-{}.yaml'.format(CONFIG_MODE))
