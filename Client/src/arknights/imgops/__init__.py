...
def compare_mse(mat1, mat2):
    """max 65025 (255**2) for 8bpc resource"""
...
def scale_pos_to_local_resolution(local_resolution: tuple, target_resolution: tuple, target_pos: tuple or list):
    x_scale = local_resolution[0] / target_resolution[0]
    y_scale = local_resolution[1] / target_resolution[1]
    if isinstance(target_pos, tuple):
        res = (int(target_pos[0] * x_scale), int(target_pos[1] * y_scale))
        return res
    else:
        res = []
        for p in target_pos:
            res.append((int(p[0] * x_scale), int(p[1] * y_scale)))
        return res
