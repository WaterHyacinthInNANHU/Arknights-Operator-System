...
from os import walk
from os.path import join, dirname, basename, isfile, splitext, isdir
...
def move(_path_: str, new_path: str):
    if _path_ == new_path:
        log('_path_ is equal to new_path, nothing changed')
        return
    ...
    for json_path in find_all_json(TEMPLATE_PATH):
        ...
        for key in _json.keys():
            if join(dir_name, key) == target_path:
                log('sure to move template {} to {}?[y/n]'.format(_path_, new_path))
                ans = None
                while ans not in ['y', 'n']:
                    ans = input()
                if ans == 'y':
                    info = _json[key]
                    original_image_path = info['path']
                    # create destination dictionary
                    dst_dir = join(TEMPLATE_PATH, dirname(dst_path))
                    ...
                    if new_key in _new_json:
                        log('template {} already exists, do you want to overwrite it?[y/n]'
                            .format(new_path))
                        ans = None
                        while ans not in ['y', 'n']:
                            ans = input()
                        if ans == 'n':
                            return
                    _, extension = os.path.splitext(info['path'])
                    info['path'] = new_path + extension
                    _new_json[new_key] = info
                    with open(new_json_path, 'w+', encoding='utf-8') as f:
                        dump(_new_json, f, indent=4)

                    # move image file
                    image_path = join(TEMPLATE_PATH, original_image_path)
                    if os.path.exists(image_path):
                        new_image_path = join(dst_dir, new_key) + extension
                        print(image_path)
                        print(new_image_path)
                        shutil.move(image_path, new_image_path)
                    else:
                        log('Error: original template image not found')
                        return
                    ...
                    log('successfully move template {} to {}'.format(_path_, new_path))
                    clear_file_tree(TEMPLATE_PATH, json_name=JSON_NAME)
                    log('cleared file tree')
                    return
                else:
                    return
    else:
        log('Error: template {} not found'.format(_path_))
