    ...
    for json_path in find_all_json(RECTANGLE_PATH):
        ...
        for key in _json.keys():
            if join(dir_name, key) == target_path:
                log('sure to move rectangle {} to {}?[y/n]'.format(_path_, new_path))
                ans = None
                ...
                if ans == 'y':
                    ...
                    dst_dir = join(RECTANGLE_PATH, dirname(dst_path))
                    ...
                    if new_key in _new_json:
                        log('rectangle {} already exists , do you want to overwrite it?[y/n]'
                            .format(new_path))
                        ans = None
                        while ans not in ['y', 'n']:
                            ans = input()
                    ...
                    log('successfully move rectangle {} to {}'.format(_path_, new_path))
        ...
        log('Error: rectangle {} not found'.format(_path_))
